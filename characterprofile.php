...
if (isset($_GET['name']) === true && empty($_GET['name']) === false) {
	...
	if ($user_id !== false) {
		...
		if ($config['ServerEngine'] == 'TFS_10') {
			if (!$loadOutfits) {
				$profile_data = user_character_data($user_id, 'account_id', 'name', 'level', 'rebirth', 'balance', 'group_id', 'vocation', 'health', 'healthmax', 'experience', 'mana', 'manamax', 'sex', 'lastlogin');
			} else { // Load outfits
				if ($config['client'] < 780) {
					$profile_data = user_character_data($user_id, 'account_id', 'name', 'level', 'rebirth', 'balance', 'group_id', 'vocation', 'health', 'healthmax', 'experience', 'mana', 'manamax', 'sex', 'lastlogin', 'lookbody', 'lookfeet', 'lookhead', 'looklegs', 'looktype');
				} else {
					$profile_data = user_character_data($user_id, 'account_id', 'name', 'level', 'rebirth', 'balance', 'group_id', 'vocation', 'health', 'healthmax', 'experience', 'mana', 'manamax', 'sex', 'lastlogin', 'lookbody', 'lookfeet', 'lookhead', 'looklegs', 'looktype', 'lookaddons');
				}
			}
		...
		<table id="characterProfileTable">
			...
			<tbody>
				...
				<tr>
					<td>Bank Balance</td>
					<td><?php echo $profile_data['balance']; ?></td>
				</tr>
				<!-- Player Rebirth -->
				<tr>
					<td>Rebirths</td>
					<td><?php echo $profile_data['rebirth']; ?></td>
				</tr>
				<!-- Player vocation -->
				<tr>
					<td>Vocation</td>
				...
				<table id="characterprofileTable" class="table table-striped table-hover">
					...
					foreach ($otherChars as $char):
						...
						<tr>
							...
							<td><?php echo (int)$char['rebirth']; ?></td>
